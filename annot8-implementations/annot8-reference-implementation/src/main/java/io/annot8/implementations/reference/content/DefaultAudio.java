...
public class DefaultAudio extends AbstractContent<AudioInputStream> implements Audio {
  ...
  public static class Builder extends AbstractContentBuilder<AudioInputStream, DefaultAudio> {
    ...
    public Content.Builder<DefaultAudio, AudioInputStream> withData(AudioInputStream data) {
      throw new Annot8RuntimeException(
          "Must use a Supplier to provider AudioInputStream, otherwise it can only be read once");
    }

    @Override
    protected DefaultAudio create(
        String id,
        String description,
        ImmutableProperties properties,
        Supplier<AudioInputStream> data) {
  ...
  public static class BuilderFactory
      extends AbstractContentBuilderFactory<AudioInputStream, DefaultAudio> {
